...
public class Assign2
{
    ...
    static void initTables()
    {
        ...
        OPTAB.put("EQU", new Opcode("EQU", "AD", 3));
        OPTAB.put("ORIGIN", new Opcode("ORIGIN", "AD", 4));
    ...
    static void pass1(String fileName) throws Exception
    {
        ...
        while ((line = br.readLine()) != null)
        {
            ...
            if (mnemonic.equals("EQU"))
            {
                if (label != null)
                {
                    Symbol ref = SYMTAB.get(op1);
                    int addr = (ref != null) ? ref.address : 0;
                    SYMTAB.put(label, new Symbol(label, addr));
                }
                ICTAB.add(new ICLine(-1, "(AD,03) (S," + op1 + ")"));
                continue;
            }
            ...
            {
                if (SYMTAB.containsKey(op1))
                    LC = SYMTAB.get(op1).address;
                else
                    LC = Integer.parseInt(op1);

                ICTAB.add(new ICLine(-1, "(AD,04) (C," + LC + ")"));
                continue;
            }
